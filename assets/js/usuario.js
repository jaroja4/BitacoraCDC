class Usuario {
  ...
  constructor(
    ...
    this.sendPasswd = sendPasswd || false;
  ...
  get clearModalNuevoUsuario() {
    ...
    $('#inp_tarjeta_fija').prop('checked', false); // Unchecks it
  ...
  get autorizador_ReadAll() {
    ...
    $.ajax({
      ...
      .done(function (e) {
        ...
        $("#sel_autorizador")
          .append(
            '<option value="null" selected disabled hidden>Seleccione un Autorizador </option>'
          )
          .change();
      })
      .fail(function (e) {
  ...
  get tramitante_ReadAll() {
    var miAccion = "ReadAll_list";

    $.ajax({
      type: "POST",
      url: "class/Tramitante.php",
      data: {
        action: miAccion,
      },
    })
  ...
  get tramitantes_ReadAll_list() {
    ...
    $.ajax({
      ...
      .done(function (e) {
        usuario.drawTramitantesDataTable(e);
      })
      .fail(function (e) {
        // dataCenter.showError(e);
      });
  ...
  get update() {
    ...
    $.ajax({
      ...
      .done(function (e) {
        ...
        usuario.clearModalNuevoUsuario;
        result
          ? usuario.SwalAlert("success", "Usuario Actualizado")
          : usuario.SwalAlert("error", "Error al Actualizar");
        $("#modal_NuevoUsuario").modal("toggle");
      })
      .fail(function (e) {});
  }

  get validarNuevoUsuario(){
    let validarUsr = true;
    ...
    if ($("#inp_identificacion").val().length<7){
    ...
    if ($("#inp_rol").val()==null){
      $("#inp_rol").addClass("red_highlights");
      validarUsr = false;
    }else{
       $("#inp_rol").removeClass("red_highlights");
    }
  ...
  get load_inp_doc(){
    ...
    $.ajax({
      ...
      .done(function (e) {
        var result = JSON.parse(e);
        $("#inp_doc").attr("src",result[0]['documento']);
        $(".downloadDoc").remove();
        $("#contrato").append(`<a class="downloadDoc" download="pdfTitle" href=${result[0]['documento']} title='Download pdf document'> Descargar </a>`);
      })
      .fail(function (e) {});
  }

  get load_img_visitante(){
    var miAccion = "load_img_visitante";
    $.ajax({
      type: "POST",
      url: "class/Usuario.php",
      data: {
      ...
      .done(function (e) {
        var result = JSON.parse(e);
        let img = result[0]['imagen']==null?usuario.img_userDefault:result[0]['imagen'];
        $("#img_visitante").attr("src",img);
        $("#inp_visitante").attr("src",img);
      })
      .fail(function (e) {});
  }
  ...
  deleteUsuario() {
    usuario.SwalAlert("error", "Opción en Diseño");
    var miAccion = "Delete";
    $.ajax({
      type: "POST",
      url: "class/Usuario.php",
      ...
      .done(function (e) {
        ...
        usuario.clearModalNuevoUsuario;
        result
          ? usuario.SwalAlert("success", "Usuario Removido")
          : usuario.SwalAlert("error", "Error al Borrar");
        $("#modal_NuevoUsuario").modal("toggle");
      })
      .fail(function (e) {});
  }
    ...
    var tb_usuarios = $("#tb_usuarios").DataTable({
      ...
      columns: [
        ...
        },
        {
          ...
          visible: true,
          mRender: function (e) {
            return '<button class=btnEliminarResponsable onclick="usuario.deleteUsuario(this)" > <i class="fa fa-trash-o" style="color:firebrick" aria-hidden="true"></i> </button>';
          },
        },
      ],
    });
  ...
  drawTramitantesDataTable(e) {
    var dataTramitantes = JSON.parse(e);
    var tb_tramitantes = $("#tb_tramitantes").DataTable({
      data: dataTramitantes,
      destroy: true,
      autoWidth: false,
      language: {
        infoEmpty: "Sin Tramitantes Creados",
        emptyTable: "Sin Tramitantes Creados",
        search: "Buscar",
        zeroRecords: "No hay resultados",
        lengthMenu: "Mostar _MENU_ registros",
        paginate: {
    ...
    $("#tb_tramitantes tbody").on("click", "tr", function () {
      usuario.clear;
      usuario.id = $("#tb_tramitantes").DataTable().row(this).data().id;
      usuario.cargarTramintantebyID;
    });
  }
}
